...
merge_w as (
    ...
    select *
    from {{ ref('atp_player_id_merge_map') }}
...
corr_w as (
    ...
    select *
    from {{ ref('atp_player_id_corrections') }}
...
name_corr_w as (
    ...
    select *
    from {{ ref('atp_player_name_corrections') }}
...
renamed as (
    ...
    select
        ...
        f.indoor,
        to_date(
            cast(f.tourney_date as varchar),
            'YYYYMMDD'
        ) as tourney_date,
        ...
        nullif(regexp_replace(trim(winner_name), '[[:space:]]+', ' '),'') as winner_name,
        ...
        -- Use the reviewed corrected name when one exists.
        -- Otherwise preserve the original source name.
        nullif(regexp_replace(trim(loser_name), '[[:space:]]+', ' '),'') as loser_name,
    ...
    left join name_corr_w as ncw
        ...
        and f.match_num = ncw.tourney_match_num
    ...
    left join name_corr_l as ncl
        ...
        and f.match_num = ncl.tourney_match_num
